...
public class SetRightCameraMethod implements ScriptMethodSPI {
    ...
    public void run() {
        ...
        Vector3f cameraPositionOffset = CameraUtils.RIGHT_OFFSET;
        Vector3f cameraLookOffset = CameraUtils.DEFAULT_LOOK_OFFSET;

        if(cameraState == null) {
            cameraModel = (ChaseCamModel)CameraModels.getCameraModel(ChaseCamModel.class);
            cameraState = new ChaseCamState(cameraPositionOffset, cameraLookOffset);
            cameraState.setDamping(1.7f);
            cameraState.setLookAtDamping(1.7f);
        }
        cameraState.setCameraPosition(translation.add(cameraPositionOffset));
